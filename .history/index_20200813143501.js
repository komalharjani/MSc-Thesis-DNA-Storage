...
function errorCorrectRedundant(cloneOne, errorDNA, cloneTwo,len) {
    ...
    let getLen = errorDNA.slice();

    let convertedOutput = removePrimers(getLen);
    let decoded = decode(convertedOutput);
    let extractedDNALength = getLength(getLen); //call getLength from here
    console.log(extractedDNALength);
