...
function onConvert() {
    ...
    if (rev) {
        ...
        //Add Primers - for Blocks
        let addedPrimers = addPrimers(dnaStringOutput);
        let reverseClone = dnaStringOutput.reverse();
        let cloneOne = reverseClone.slice();

        //ERROR SIMULATOR
        let cloneError = errorSimulator(reverseClone);
        let cloneTwoError = errorSimulator(cloneOne);
        let dnaStringError = errorSimulator(addedPrimers);

        //CONVERT REVERSE BACK
        cloneError.reverse();
        cloneTwoError.reverse();
        dnaStringError.reverse();

        //ERROR CORRECT WITH REV COMPLEMENT
        let errorCorrection = errorCorrectRedundant(dnaStringError, reverseClone, cloneOne);
        console.log(errorCorrection);
