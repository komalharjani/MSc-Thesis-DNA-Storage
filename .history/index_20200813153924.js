...
function onSubmit() {
    ...
    let errorCorrectionResult = dnaToBinary(dnaStringError);
...
function errorCorrectRedundant(cloneOne, errorDNA, cloneTwo,len) {
    ...
    let conv = dnaToBinary(getLenClone);
    let remPri = removePrimers(conv);
    let dec = decode(remPri);
    let bin = binaryToDNA(dec);
    let ret = getLength(bin);
    console.log(ret);
    //binaryToText(dec);
