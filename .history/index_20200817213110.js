...
function onConvert() {
    ...
    if (blocks) {
        ...
        let n = 30;
        let addBlock = addBlocks(dnaStringOutput, n).join("TTTT");
        let blockArr = Array.from(addBlock);
        console.log(blockArr);
        // let addedPrimers = addPrimers(blockArr);

        //Create Clones
        let cloneOne = blockArr.slice();
        let cloneTwo = blockArr.slice();

        //Error Simulator
        let cloneError = errorSimulator(cloneOne);
        let cloneTwoError = errorSimulator(cloneTwo);
        let dnaStringError = errorSimulator(blockArr);

        console.log(cloneError);
        console.log(cloneTwoError);
        console.log(dnaStringError);

        let res = fixedBlock(cloneOne, cloneTwo, blockArr);
...
function addBlocks(dnaArray, n) {
    ...
    for (let i = 0; i < dnaArray.length; i += n) {
