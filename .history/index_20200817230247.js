...
function onConvert() {
    ...
    if (blocks) {
        ...
        let addBlock = addBlocks(dnaArray, lengthOfBlock).join("TTTT");
        let blockArr = Array.from(addBlock);
        console.log(blockArr);
        //let primedOriginal = addPrimers(blockArr);

        //Create Clones
        let cloneOneString = Array.from(addBlock.slice());
        let cloneTwoString = Array.from(addBlock.slice());

        console.log(blockArr);
        for(let i = 5; i < blockArr.length + 1; i += 8) {
            console.log()
                blockArr.splice(i,4);
                cloneOneString.splice(i,4);
                cloneTwoString.splice(i,4);
    
        }

        console.log(blockArr);
        console.log(cloneOneString);
        console.log(cloneTwoString);

        //Error Simulator
        let corruptClone = errorSimulator(cloneOneString.join(""));
        let corruptCloneTwo = errorSimulator(cloneTwoString.join(""));
        let corruptOriginal = errorSimulator(blockArr.join(""));
...
function addBlocks(dnaArray, n) {
    ...
    let str = dnaArray.join("");
    let block = [];

    for (let i = 0; i < dnaArray.length; i += n +1) {
        block.push(str.substr(i, n))
    }
    return block;
}
