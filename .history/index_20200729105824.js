...
function errorSimulator(dnaString) { //does this have to deal with a string?
    ...
    console.log(dnaString.split("")); //convert into array
    for (let i = 0; i < errorRate + 1; i++) { //for error rate
        var item = dnaString[Math.floor(Math.random() * dnaString.length)]; //generate random index to be deleted
        console.log(item); 
        let itemIndex = dnaString.indexOf(item);
        console.log(itemIndex);
        console.log(dnaString);
        //dnaString.splice(itemIndex, 1); //remove element from string here
    }
    console.log(dnaString);
