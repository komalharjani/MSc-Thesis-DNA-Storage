...
function similarText(originalText, recoveredText) {
    ...
    let pos2 = 0;
    let max = 0;
    let firstLength = originalText.length;
    let secondLength = recoveredText.length
    let sum;
    ...
    for (let i = 0; i < firstLength; i++) {
        for (let j = 0; j < secondLength; j++) {
            for (let k = 0;
                (i + k < firstLength) && (j + k < secondLength) && (originalText.charAt(i + k) === recoveredText.charAt(j + k)); k++);
            if (k > max) {
                max = k;
                pos1 = i;
                pos2 = j;
    ...
    if (sum) {
        ...
        if ((pos1 + max < firstLength) && (pos2 + max < secondLength)) {
            sum += this.similarText(originalText.substr(pos1 + max, firstLength - pos1 - max), recoveredText.substr(pos2 + max, secondLength - pos2 - max));
        }
    }
