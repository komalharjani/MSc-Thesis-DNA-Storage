...
function onConvert() {
    ...
    errorDisplay.innerHTML = "Specified Error Rate: " + noErrors + " Percent";
...
function errorSimulator(dnaString) { //does this have to deal with a string?
    ...
    while (noErrors > 0) { //for error rate
        ...
        if (del) {
            dnaArray.splice(randomIndex, noDelete);
            // console.log(dnaArray);
            errorRate - noDelete;
        }
        //insertion error
        ...
        if (insert) {
            dnaArray.splice(randomIndex, noInsert, randomElement);
            errorRate - noInsert;
        }
        //substitution error
        let sub = document.getElementById("sub").checked;
        if (sub) {
            dnaArray.splice(randomIndex, noSub, randomElement);
            noDelete - noInsert;
        }
    }
    return dnaArray;
