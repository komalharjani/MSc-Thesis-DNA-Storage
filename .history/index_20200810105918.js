...
function errorCorrection(cloneError,dnaStringError,errorComp) {
    ...
    for(let i in cloneError) {
        if(!dnaStringError.includes(cloneError[i])) {
             console.log("hi");
            temp.push({
                base: cloneError[i],
                index: i
    ...
    for(let j in dnaStringError) {
        if(!cloneError.includes(dnaStringError[j])) {
            temp.push({
                base: dnaStringError[j],
                index: j
            })
        }
    }
    ...
    var difference = cloneError.filter(x => !dnaStringError.includes(x));
