...
function onConvert() {
    ...
    if (blocks) {
        ...
        //Create Clones
        let cloneOneString = primedOriginal.slice();
        let cloneTwoString = primedOriginal.slice();

        //Error Simulator
        let corruptClone = errorSimulator(cloneOneString);
        let corruptCloneTwo = errorSimulator(cloneTwoString);
        let corruptOriginal = errorSimulator(primedOriginal);
...
function errorCorrectRedundantBlock(errorDNA, cloneOne, cloneTwo, len) {
    ...
    for(let i=5; i < errorDNA.length; i += 5) {
            console.log(errorDNA[i]);
    }
    console.log(errorDNA);
